...
This project supports ride notifications through Google Firebase Cloud Messaging.

Set these environment values in backend `.env`:
...
- `POST /users/register-fcm-token` (auth user)
- `POST /captains/register-fcm-token` (auth captain)

Body:

- `fcmToken` (string)

Ride notification events sent via Firebase:

- Ride confirmed
- Ride started
- 100m left to destination
- Ride ended
